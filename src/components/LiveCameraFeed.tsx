...
export default function LiveCameraFeed({
  ...
  const syncCameraSettings = (newSrc: 'device' | 'ip_webcam', newUrl: string, newMode: 'direct' | 'proxy') => {
    if (typeof window !== 'undefined') {
      ...
      localStorage.setItem('vikrant_ip_stream_mode', newMode);
      window.dispatchEvent(
        new CustomEvent('vikrant:camera_settings_changed', {
          detail: { source: newSrc, url: newUrl, mode: newMode },
        })
      );
    }
  };
  ...
  return (
    <div className={`relative bg-black rounded-xl overflow-hidden border border-panel-border flex items-center justify-center group ${className}`}>
      ...
      {feedSource === 'device' && hasCamera ? (
        ...
        />
      ) : feedSource === 'ip_webcam' && ipWebcamUrl ? (
        ...
        <div className="w-full h-full relative flex flex-col items-center justify-center bg-zinc-950">
          <video
            autoPlay
            loop
            muted
            playsInline
            className={`w-full h-full object-cover ${
              mode === 'thermal' ? 'filter contrast-150 hue-rotate-180 saturate-200' : 'opacity-60 grayscale'
            }`}
          >
            <source src="https://www.w3schools.com/html/mov_bbb.mp4" type="video/mp4" />
          </video>
          <div className="absolute inset-0 bg-black/65 flex flex-col items-center justify-center p-4 text-center z-10">
            <Radio size={26} className="text-cyan-400 mb-2 animate-pulse" />
            <span className="text-[11px] font-mono text-foreground/90 uppercase tracking-widest font-bold">
              {feedSource === 'ip_webcam' ? 'PHONE IP WEBCAM STREAM' : 'LAPTOP / USB WEBCAM'}
            </span>
            <span className="text-[9px] font-mono text-foreground/50 mt-1 max-w-[220px]">
              {cameraError || 'Hold up any bottle or bag — instant auto-capture will fire!'}
            </span>
              ...
              {feedSource === 'ip_webcam' ? (
                <>
                  <button
                    ...
                    className="px-3 py-1.5 liquid-btn text-cyan-300 font-mono text-[10px] rounded-lg font-bold"
                  >
                    TOGGLE {ipStreamMode === 'direct' ? 'PROXY' : 'DIRECT'}
                  </button>
                  <button
                    onClick={() => setShowIpInput(true)}
                    className="px-3 py-1.5 liquid-btn-primary text-white font-mono text-[10px] rounded-lg font-bold"
                  >
                    CONFIGURE IP
                  </button>
                </>
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 liquid-btn-primary text-white font-mono text-[10px] rounded-lg font-bold"
                >
                  ACTIVATE CAMERA
                </button>
              )}
